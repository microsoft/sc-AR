...
## Abstract
While machine learning models trained on single-cell transcriptomics data have shown great promise in providing biological insights, existing tools struggle to effectively model underrepresented and out-of-distribution features or cell states, which are often the most biologically relevant. We present an Adaptive Resampling (AR) approach that addresses these limitations and enhances single-cell representation learning by resampling data based on its learned latent structure in an online, adaptive manner concurrent with model training.Experiments on cell type classification, gene expression reconstruction, and the perturbation response prediction tasks demonstrate that the proposed AR approach leads to significant downstream performance improvements across datasets, tasks, and metrics. Additionally, it enhances the quality of learned cellular representations compared to standard training methods. Our results suggest that AR may serve as a valuable technique for improving representation learning and predictive performance in single-cell transcriptomic models.
...
You can use the implemented function `update_resampling_weights()` located in `scripts/scvi_train.py`.
