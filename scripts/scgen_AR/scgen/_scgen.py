...
from scripts.utils import create_optimizers
from ._dataloader import Dataset
...
class SCGEN(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    """
    Implementation of scGen model for batch removal
    ...
    adata
        ...
        meth:`~scgen.SCGEN.setup_anndata`.
    n_hidden
    ...
    **model_kwargs
    ...
    def monitor(self, data, label, args):

        data_dict = {}
        data_dict['X'] = data
        if args.gpu:
            data_dict['X'] = data_dict['X'].cuda()
        out = self.module(
            data_dict,
            compute_loss=True)
        inference_outputs_valid = out[0]
        generative_outputs_valid = out[1]
        loss = self.module.loss(
            data_dict,
            inference_outputs_valid,
            generative_outputs_valid)

        return loss.loss

    def prepare_data_for_training(self, args, adata):
        ...
        Returns:
            ...
            loader_train (DataLoader): DataLoader object containing the training data
            optimizer (torch.optim): optimizer object for training the model"""

        # setup train and validation adata
        ...
        dataset = Dataset(
            adata=train_adata,
            groups=train_adata.obs[args.adata_label_cell])
        ...
        # update validation adata if in_dist_group is provided
        if args.in_dist_group != '':
            ...
            not_valid_index = \
                ...
                if x not in valid_index]
            in_dist_valid_adata = in_dist_adata[valid_index]
            in_dist_not_valid_adata = in_dist_adata[not_valid_index]
            
            new_valid_adata = in_dist_valid_adata.concatenate(other_adata)
            
            assert valid_adata.shape[0] == in_dist_valid_adata.shape[0] +\
                other_adata.shape[0] + in_dist_not_valid_adata.shape[0]
            valid_adata = new_valid_adata
            
        return train_adata, valid_adata, dataset, loader_train, optimizer
    ...
    def mytrain_AR(
        ...
        # address = root+"saved_models/"+args.data + \
        #     "/seed"+str(args.seed) + \
        #     '/'+args.id+"-epoch1500.pt"
        ...
        self.set_seed(args.seed)
        train_adata, valid_adata, dataset, loader_train, optimizer = \
            self.prepare_data_for_training(args, adata)
        ...
        for epoch in range(args.num_epoch):
            # project all samples into the latent space
            # and update sample weights
            with torch.no_grad():
                train_adata_X = torch.tensor(train_adata.X.toarray())
                if args.gpu:
                    train_adata_X = train_adata_X.cuda()
                inference_out = self.module.inference(
                    train_adata_X)
                z = inference_out['z'].cpu()
                dataset.update_training_sample_probabilities(
                    z, args.bins, args.alpha)
                
                if args.debug:
                    # plot histogram of w
                    pyplot.figure()
                    fig = pyplot.hist(2**dataset.w, bins=100)
                    pyplot.savefig(root+'debug/'+args.id+'/w_hist_epoch'+str(epoch)+'.png')
            ...
            while seen_sample < len(dataset):

                if seen_sample + args.batch_size <= len(dataset):
                    batch_data = dataset.resample(args.batch_size, args)
                    seen_sample += args.batch_size

                elif seen_sample + args.batch_size > len(dataset):
                    remaining_sample = len(dataset) - seen_sample
                    batch_data = dataset.resample(remaining_sample, args)
                    seen_sample += remaining_sample

                else:
                    print(seen_sample, len(dataset))
                    raise ValueError('Invalid value for seen_sample')

                steps += 1
                if args.gpu:
                    batch_data['X'] = batch_data['X'].cuda()

                # Zero gradients for every batch
                optimizer.zero_grad()

                # Make predictions for this batch
                pred = self.module(batch_data)
                inference_outputs = pred[0]
                generative_outputs = pred[1]
            ...
            with torch.no_grad():
                ...
                self.save(root+"saved_models/"+args.data +
                          "/seed"+str(args.seed) +
                          '/'+args.id+".pt",
                          overwrite=True,
                          attributes={'epoch':epoch})

                current_valid_loss = self.monitor(
                    torch.tensor(valid_adata.X.toarray()),
                    'Valid Loss (AR)', args)
                valid_loss_list.append(float(current_valid_loss))
                wandb.log({'Epoch': epoch,
                           'Train Loss (AR)': epoch_loss,
                           'Valid Loss (AR)': current_valid_loss})
            ...
            if epoch % args.checkpoint == 0:
                self.save(root+"saved_models/"+args.data +
                          "/seed"+str(args.seed) +
                          '/'+args.id+"-epoch"+str(epoch)+".pt",
                          overwrite=True)

        self.is_trained = True
        file = root+'result/train/'+args.data+'/' + \
            "{:%Y%m%d}".format(datetime.now())+'_'+args.id + \
            '_train_loss.csv'
        np.savetxt(file, np.array(train_loss_list, dtype=float), delimiter=',')

        file = root+'result/train/'+args.data+'/' + \
            "{:%Y%m%d}".format(datetime.now())+'_'+args.id + \
            '_valid_loss.csv'
        np.savetxt(file, np.array(valid_loss_list, dtype=float), delimiter=',')

        with open(root+"saved_models/"+args.data +
                  "/seed"+str(args.seed)+'/config/'+
                  args.id+"_wandb_config.yaml", "a") as f:
    ...
    def mytrain_naive(
        ...
        for epoch in range(args.num_epoch):
            ...
            with torch.no_grad():
                ...
                current_valid_loss = self.monitor(
                    torch.tensor(valid_adata.X.toarray()),
                    'Valid Loss (Naive)', args)
                valid_loss_list.append(float(current_valid_loss))
                wandb.log({'Epoch': epoch,
                           'Train Loss (Naive)': epoch_loss,
                           'Valid Loss (Naive)': current_valid_loss})
        ...
        file = root+'result/train/'+args.data+'/' + \
            "{:%Y%m%d}".format(datetime.now())+'_' + \
            args.id+'_valid_loss.csv'
        np.savetxt(file, np.array(valid_loss_list, dtype=float), delimiter=',')
