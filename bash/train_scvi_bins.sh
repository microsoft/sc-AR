...
bins=${13}         # number of histogram bins for AR resampling weights
...
if grep -q "main.py: error:" ${log_path}/$today/${today}-${data}-AR${AR}-ood${ood}-seed${seed}-epoch${num_epoch}-${model_name}-${atlas_count}atlas-alpha${alpha}-bins${bins}-bs${batch_size}-train.err; then
    echo "Error"
fi
