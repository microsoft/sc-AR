...
resampling_weight_path=$6 # path to the folder for saving the resampling weights ending with /resampling_weights/
  ...
  for atlas_count in "${atlas_counts[@]}"; do
    for t in "${tracked_epoch[@]}"; do
      ...
      "${SCRIPT_DIR}/store_resampling_weights_scvi.sh" \
        ...
        "${root}" \
        "${atlas_count}" \
        "${resampling_weight_path_current}" >> "${log_file}" 2> "${err_file}"
