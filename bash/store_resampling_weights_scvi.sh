...
if [ "$#" -lt 15 ]; then
  echo "Usage: $0 <data> <model> <num_epoch> <variable_con> <con_percent> <in_dist_group> <seed> <tracked_epoch> <model_name> <data_path> <out_path> <latent_dim> <root> <atlas_count> <resampling_weight_path>"
  exit 1
fi
...
root=${13}
atlas_count=${14}
resampling_weight_path=${15}

## Sample command
# ./store_resampling_weights_scvi.sh \
#     sctab \
#     AR \
#     300 \
#     False \
...
python "${SCRIPT_DIR}/../main.py" \
    ...
    --store_resampling_weights True \
    --resampling_weight_path "${resampling_weight_path}" \
