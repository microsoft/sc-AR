    ...
    for in_dist_group in "${train_array[@]}"; do
        for s in "${seed[@]}"; do
            ...
            python ../main.py \
                ...
                --data_path ${data_path} \
                --out_path ${out_path} \
                --seed ${s} >> ./log/$today/${today}-standard-${data}-test-${list[$i]}-${model}-OOD${ood}-epoch${num_epoch}-varcon${variable_con}-conper${con_percent}-seed${s}-indist${in_dist_group}-test.txt 2> ./log/$today/${today}-standard-${data}-test-${list[$i]}-${model}-OOD${ood}-epoch${num_epoch}-varcon${variable_con}-conper${con_percent}-seed${s}-indist${in_dist_group}-test.err

            if grep -q "Traceback" ./log/$today/${today}-standard-${data}-test-${list[$i]}-${model}-OOD${ood}-epoch${num_epoch}-varcon${variable_con}-conper${con_percent}-seed${s}-indist${in_dist_group}-test.err; then
                echo "Error"
            fi

            if grep -q "main.py: error: " ./log/$today/${today}-standard-${data}-test-${list[$i]}-${model}-OOD${ood}-epoch${num_epoch}-varcon${variable_con}-conper${con_percent}-seed${s}-indist${in_dist_group}-test.err; then
                echo "Error"
            fi
